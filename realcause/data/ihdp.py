...
def load_ihdp(
    ...
    if split == "all":
        train, test = load_ihdp_datasets(
            split=split, n_realizations=n_realizations, dataroot=dataroot
        )
    else:
        data = load_ihdp_datasets(
            split=split, n_realizations=n_realizations, dataroot=dataroot
        )
    ...
    if observe_counterfactuals:
        ...
        ites = np.concatenate(
            [ites, ites.copy()]
        )  # comment if you don't want duplicates
    else:
        d["w"] = w
        d["t"] = t
        d["y"] = y
    ...
    if n_realizations == 100:
        if split == "train" or split == "all":
            path = download_dataset(
                IHDP_100_TRAIN_URL, "IHDP train 100", dataroot=dataroot
            )
            train = np.load(path)
        if split == "test" or split == "all":
            path = download_dataset(
                IHDP_100_TEST_URL, "IHDP test 100", dataroot=dataroot
            )
            test = np.load(path)
    elif n_realizations == 1000:
        if split == "train" or split == "all":
            path = download_dataset(
                IHDP_1000_TRAIN_URL, "IHDP train 1000", dataroot=dataroot
            )
            unzip_path = unzip(path)
            train = np.load(unzip_path)
        if split == "test" or split == "all":
            path = download_dataset(
                IHDP_1000_TEST_URL, "IHDP test 1000", dataroot=dataroot
            )
            unzip_path = unzip(path)
